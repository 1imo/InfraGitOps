...
public class NginxWorkflowOrchestration : IWorkflowOrchestration
{
    ...
    public NginxWorkflowOrchestration(
        IApplier applier,
    ...
    {
        Console.WriteLine($"[NginxWorkflowOrchestration] Starting workflow for {ComponentName}");
    ...
    private async Task<object> ProcessTransformersAsync(object manifestData)
    {
        ...
        {
            Console.WriteLine($"[NginxWorkflowOrchestration] Running transformer: {transformer.SourceComponent} → {transformer.TargetComponent}");
